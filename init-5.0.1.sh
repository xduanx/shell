...
#                 print a cow and look more lovely
#                 add gcc,pcre-devel,openssl-devel,zlib-devel
#                 fix query num and summerize num
#                 make echo info brifier and replace echo with printf,make info show better
#                 make printf and query info more convenient with array
...
printf {"%3s","1:"}
if [ "$s_selinux" == "Disabled" -o "$s_selinux" == "Permissive" ];then
...
if [ -z "$s_vim" ];then
    ...
    echo -e "\033[32m   set vim:set shitwidth=4\033[0m"
else
...
startNO=10
#variable "apps" for function "query" and "summerize"
#add app to install here
apps=(lftp createrepo net-tools gcc pcre-devel openssl-devel zlib-devel)
for i in ${apps[*]};do
    let startNO=$startNO+1
    printf {"%3s","$startNO:"}
    query $i
done
...
call4(){
    ...
    mv /etc/yum.repos.d/* /etc/yum.repos.d/default/
...
call10(){
    #alter hostname
    echo -n "alter hostname or not,0(no),1(yes):"
    while true;do
	read choice
	if [ "$choice" == 1 ];then
	    read host_name
	    ...
	    echo -e "\033[31mtype 1 or 0, please\033[0m"
	    echo -n "alter hostname or not,1(yes),0(no):"
	fi
    done
}
...
#summary
echo -e "\033[32m1:SELINX is $(getenforce) \033[0m"
echo -e "\033[32m2:FIREWALL $(systemctl status firewalld.service | sed -n '3p'|awk -F: '{print $2}'|awk '{print $1}') \033[0m"
echo -e "\033[32m3:IP $(ip address show eno16777736 | grep "brd" | sed '1d'|awk '{print $2}') by $(nmcli connection show eno16777736 | grep "ipv4.method"|awk '{print $2}') \033[0m"
echo -e "\033[32m4:YUM(dvd) is enabled and there are $(yum repolist iso | sed -n '/iso/p' |awk '{print $NF}') software \033[0m"
echo -e "\033[32m5:DVD mounted on $(df -hT | grep "iso9660" |awk '{print $NF}') \033[0m"
echo -e "\033[32m6:$(rpm -qa |grep vim-enhanced) is installed \033[0m"
echo -e "\033[32m7:$(rpm -qa |grep bash-completion) is installed \033[0m"
echo -e "\033[32m8:$(sed -n -e '93s/^/SSH:/p' -e '129s/^/\ \ SSH:/p' /etc/ssh/sshd_config) \033[0m"
echo -e "\033[32m10:hostname is $(hostname) \033[0m"

sum_startNO=10
for i in ${apps[*]};do
    ((sum_startNO=$sum_startNO+1))
    echo -n "$sum_startNO:"
    summerize $i
done
